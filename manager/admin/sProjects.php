...
function updateSingle($pdo, $r){
    ...
    if($oldStatus!=STT_DA_KY && $r->status == STT_DA_KY){
        ...
        $stmt = $pdo->prepare($sql);
        $tmp = $stmt->execute(array($r->PID, $user['ID'], 'Tiếp Nhận Đơn Hàng', 1, $cur));
        //error_log(print_r($tmp, true));die;

        $sql = 'INSERT INTO timeline(PID, UID, task, finish) VALUES(?,?,?,?)';
        $stmt = $pdo->prepare($sql);
        $tmp = $stmt->execute(array($r->PID, 0, 'Xử Lý File', 0));

        $sql = 'INSERT INTO timeline(PID, UID, task, finish) VALUES(?,?,?,?)';
        $stmt = $pdo->prepare($sql);
        $tmp = $stmt->execute(array($r->PID, 0, 'In Ấn', 0));

        $sql = 'INSERT INTO timeline(PID, UID, task, finish) VALUES(?,?,?,?)';
        $stmt = $pdo->prepare($sql);
        $tmp = $stmt->execute(array($r->PID, 0, 'Gia Công Thành Phẩm', 0));

        $sql = 'INSERT INTO timeline(PID, UID, task, finish) VALUES(?,?,?,?)';
        $stmt = $pdo->prepare($sql);
        $tmp = $stmt->execute(array($r->PID, 0, 'Giao Hàng', 0));
...
if( isset($_REQUEST['list'])){
   ...
   // if (!isset($_SESSION["Projects"]))
    //{
        ...
        $sql = "Select * From projects WHERE PID>? AND status!=0 ORDER BY PID ASC";
        
        $stmt = $dbh->prepare($sql);
        $stmt->execute(array($maxPID-100));
